<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:47:"../application/index/view/wap/wap/wap/doci.html";i:1560070719;s:53:"../application/index/view/wap/wap/include/header.html";i:1560065679;}*/ ?>
<!doctype html>
<html>
<head>
...
<div id="index_hm_text">
    ...
    <i>发布时间：<?php echo $data['time']; ?></i>
    <center class="doc-img">
        <img src="<?php echo $data['img']; ?>" width="228" height="250"/>
    </center>
    <label class="con1_nr" style="text-align: left;" id="intro">
        <p class="p"> 简&nbsp;&nbsp;&nbsp;&nbsp;介：</p>
        <?php if(!empty($data['intro'])): ?><?php echo $data['intro']; endif; ?>
    </label>
    <label class="con1_nr" style="text-align: left;">
        <p class="p"> 荣&nbsp;&nbsp;&nbsp;&nbsp;誉：</p>
        <?php if(!empty($data['honour'])): ?><?php echo $data['honour']; endif; ?>
    </label>

     <label class="con1_nr" style="text-align: left;">
        <p class="p"> 擅长项目：</p>
         <?php if(!empty($data['be_expert_in'])): if(is_array($data['be_expert_in']) || $data['be_expert_in'] instanceof \think\Collection || $data['be_expert_in'] instanceof \think\Paginator): $k = 0; $__LIST__ = $data['be_expert_in'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($k % 2 );++$k;?>
         <span style="border-radius: 2px;font-size: 15px; color: #988565;"> |<?php echo $v; ?></span>
         <?php endforeach; endif; else: echo "" ;endif; endif; ?>
    </label>
    ...
    <div class="con1_fxlx doc-li">
        <?php if(!empty($doc)): if(is_array($doc) || $doc instanceof \think\Collection || $doc instanceof \think\Paginator): $k = 0; $__LIST__ = $doc;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?>
            <a href="<?php echo url('/wap/doci', ['id' => $vo['id']]); ?>"><?php echo $vo['name']; ?></a>
            <?php endforeach; endif; else: echo "" ;endif; endif; ?>
    </div>
</div>
